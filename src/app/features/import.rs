...
/// Resizes raw image `bytes` to a 16x16 RGBA grid (bytes-based, for WASM drops).
...
/// - `String`: path/name of the original Crumbicon file, or modified path
///   (`.svg`) for images that were not already an SVG/Crumbicon.
pub fn import_bytes(
    file_name: &str,
    bytes: &[u8],
